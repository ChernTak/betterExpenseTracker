...
// Take the last currency-shaped number on the line, since qualifier text can sit between the label and the number.
...
// Malaysia rounds cash payments to the nearest 5 sen; the minus sign lives in the numeric group (not a "[:\-]?" separator class) so a bare "Rounding -0.05" without a colon isn't mistaken for the separator.
...
// Prefers a line that looks like a total; falls back to the largest currency-shaped number on the page.
function extractAmount(text) {
  const lines = text.split('\n');

  // \btotal\b naturally excludes "subtotal" since there's no word boundary between "sub" and "total".
...
// Matches generically since Malaysian receipts label this "Tax", "SST", "GST" or "Service Tax" depending on era/regime.
...
function extractLineAmount(lines, keywordRegex, { excludeTotalLines = false } = {}) {
  for (const line of lines) {
    ...
    // Screens out total lines that mention tax in passing (e.g. "Total (incl Tax) 15.98"), not subtotal lines.
...
// Unlike applyRounding, this scans the whole receipt since cross-validation only needs to know if rounding was printed at all.
...
// Cross-checks the grand total against subtotal+tax+rounding to catch a misread digit; returns nulls (not invalid) when there's no full breakdown to check against.
...
// Prefers the trading/brand name over the registered legal entity name that Malaysian receipts often print above it.
...
// Must exclude website/email lines here so the loop falls through to extractDomainMerchant instead of returning the raw URL.
...
const isNoisyNameLine = (line) =>
  ...
  // Stops receipts with an all-noise header from returning a price row before we ever consult the domain fallback.
  TOTAL_WORD_REGEX.test(line);

// Pulls the brand from the receipt's own website/email domain instead of maintaining a hardcoded legal-entity -> brand lookup table.
...
function extractDomainMerchant(text) {
  ...
  // Title-case what's left after stripping "www."/".com"/country-code suffixes, e.g. "mcdonalds" -> "Mcdonalds".
...
function extractMerchant(text) {
  ...
  // OCR sometimes splits one printed line across several recognized lines, so check a legal-entity suffix a few lines below too.
  ...
  // Stays scoped to the same header region as the backward scan above rather than scanning the whole receipt body.
  ...
  // Fallback for when no header line reads as a clean outlet name: use the receipt's own domain to name the brand.
...
// POST /api/ocr/parse — pulls merchant/date/amount from on-device ML Kit text and records an audit row (FR4.3); no image is uploaded or stored.
exports.parseReceipt = async (req, res) => {
  ...
  // Arithmetic cross-check (FR4.3): isMathValid is null (not false) when the receipt doesn't itemize enough to check.
  ...
  // ML Kit doesn't expose a confidence score like Cloud Vision does, so proxy it with how many of the three fields we found.
  ...
  try {
    ...
    return res.status(200).json({
      ...
      amount,
      confidence,
      // false means the total didn't reconcile with subtotal+tax+rounding; computedTotal is offered as a corrected suggestion.
      isMathValid,
      computedTotal,
    });
  } catch (err) {
